...
#    print("label is : {} <----> predict is : {}".format(text, predict_text))
    print("predict is : {}".format(predict_text))

def new_sample():
    img = Image.open('/home/binyang_ni/python/Python/tensorflow/test_for_tf/Captcha/apply_model/new_img2.jpg')
    basewidth = config.IMAGE_WIDTH
    baseheight = config.IMAGE_HEIGHT
    re_img = img.resize((basewidth, baseheight), PIL.Image.ANTIALIAS)
    new_image = np.array(re_img)
